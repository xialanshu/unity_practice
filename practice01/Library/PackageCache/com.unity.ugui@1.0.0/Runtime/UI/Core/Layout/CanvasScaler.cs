...
namespace UnityEngine.UI
{
    ...
    public class CanvasScaler : UIBehaviour
    {
        ...
        protected virtual void HandleScaleWithScreenSize()
        {
...
#if UNITY_HMIANDROID && !UNITY_EDITOR
            if (displayIndex >= 0 && displayIndex < Display.displays.Length && !m_Canvas.useForceUpdateSize)
#else
            ...
            float scaleFactor = 0;
            float scaleWidth = screenSize.x / m_ReferenceResolution.x;
            float scaleHeight = screenSize.y / m_ReferenceResolution.y;
            switch (m_ScreenMatchMode)
            {
                case ScreenMatchMode.MatchWidthOrHeight:
                {
                    ...
                    if (m_MatchWidthOrHeight == 0.0f)
                    {
                        scaleFactor = scaleWidth;
                    }
                    else if (m_MatchWidthOrHeight == 1.0f)
                    {
                        scaleFactor = scaleHeight;
                    }
                    else
                    {
                        float logWeightedAverage = Mathf.Lerp(Mathf.Log(scaleWidth, kLogBase), Mathf.Log(scaleHeight, kLogBase), m_MatchWidthOrHeight);
                ...
                {
                    scaleFactor = Mathf.Min(scaleWidth, scaleHeight);
                    break;
                }
                case ScreenMatchMode.Shrink:
                {
                    scaleFactor = Mathf.Max(scaleWidth, scaleHeight);
                    break;
                }
            }
