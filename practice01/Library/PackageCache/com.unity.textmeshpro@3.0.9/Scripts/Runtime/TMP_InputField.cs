...
namespace TMPro
{
    ...
    public partial class TMP_InputField : Selectable,
        ...
        IScrollHandler
    {
        ...
        private bool InPlaceEditing()
        {
            if (Application.platform == RuntimePlatform.WSAPlayerX86 || Application.platform == RuntimePlatform.WSAPlayerX64 || Application.platform == RuntimePlatform.WSAPlayerARM
                || Application.platform == RuntimePlatform.WeixinMiniGamePlayer || Application.platform == RuntimePlatform.WebGLPlayer)
        ...
        protected virtual void LateUpdate()
        {
            ...
            else if (m_HideMobileInput && (Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.OpenHarmony || Application.platform == RuntimePlatform.HMIAndroid))
            {
                UpdateStringPositionFromKeyboard();
            }
        ...
        public virtual void OnDrag(PointerEventData eventData)
        {
            ...
            else
            {
                if (insertionSide == CaretPosition.Left)
                {
                    ...
                    if (m_TextComponent.enableAdvanceTextProcess) 
                        stringSelectPositionInternal = insertionIndex == 0 
                            ? m_TextComponent.textInfo.characterInfo[0].index
                ...
                {
                    stringSelectPositionInternal = m_TextComponent.textInfo.characterInfo[insertionIndex].index + m_TextComponent.textInfo.characterInfo[insertionIndex].stringLength;
                    // mouse click should base on 'visual'
                    if (m_TextComponent.enableAdvanceTextProcess)
                        if (!m_TextComponent.isRightToLeftText)
                        {
                            stringSelectPositionInternal = m_TextComponent.textInfo.characterInfo[insertionIndex + 1].index;
                        }
                        else
                        {
                            int targetVisual = Math.Max(0, insertionIndex - 1);
                            stringSelectPositionInternal = m_TextComponent.textInfo.characterInfo[targetVisual].index;
                        }
                    ...
                    else
                    {
                        ...
                        else if (insertionSide == CaretPosition.Right)
                        {
                            ...
                            if (m_TextComponent.enableAdvanceTextProcess)
                                if (!m_TextComponent.isRightToLeftText)
                                {
                                    stringPositionInternal = stringSelectPositionInternal = m_TextComponent.textInfo.characterInfo[insertionIndex + 1].index;
                                }
                                else
                                {
                                    var targetVisual = Math.Max(0, insertionIndex - 1);
                                    stringPositionInternal = stringSelectPositionInternal = m_TextComponent.textInfo.characterInfo[targetVisual].index;
                                }
                        }
                    }
                }
        ...
        private void MoveLeft(bool shift, bool ctrl)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (m_TextComponent.enableAdvanceTextProcess)
                        position = m_TextComponent.textInfo.characterInfo[caretSelectPositionInternal].prevStringIndex;
        ...
        private void Backspace()
        {
            ...
            else
            {
                ...
                {
                    // characterInfo is visual order, but delete should be logic order
                    if (m_TextComponent.enableAdvanceTextProcess)
                    {
                        var characterToRemove = m_TextComponent.textInfo.characterInfo[caretPositionInternal].prevStringIndex;
                        // we set stringIdx 0 's prevStringIndex 0, this is ok in move, but backspace should delete itself.
                        if (characterToRemove != stringSelectPositionInternal)
                        {
                            int caretToRemove = GetCaretPositionFromStringIndex(characterToRemove);
                        
                            int numberOfCharactersToRemove = m_TextComponent.textInfo.characterInfo[caretToRemove].stringLength;
                            m_Text = text.Remove(m_TextComponent.textInfo.characterInfo[caretToRemove].index, numberOfCharactersToRemove);
                            stringSelectPositionInternal = stringPositionInternal = caretPositionInternal < 1
                                ? m_TextComponent.textInfo.characterInfo[0].index
                                : m_TextComponent.textInfo.characterInfo[caretToRemove].index;
        ...
        private int GetCaretPositionFromStringIndex(int stringIndex)
        {
            ...
            if (m_TextComponent.enableAdvanceTextProcess)
            {
                ...
                int bestCarentPosition = count;
                for (int i = 0; i < count; i++)
                {
                    var ithStringIndex = m_TextComponent.textInfo.characterInfo[i].index;
                    if (ithStringIndex >= stringIndex && ithStringIndex - stringIndex < minDis)
                    {
                        minDis = ithStringIndex - stringIndex;
                        bestCarentPosition = i;
                    }
                }
                return bestCarentPosition;
            }
        ...
        private void GenerateCaret(VertexHelper vbo, Vector2 roundingOffset)
        {
            ...
            if (m_TextComponent.enableAdvanceTextProcess)
            {
                ...
                if (currentCharacter.textElement.bidiDir == 1)
                {
                    startPosition.x = currentCharacter.topRight.x;
                }
            }
